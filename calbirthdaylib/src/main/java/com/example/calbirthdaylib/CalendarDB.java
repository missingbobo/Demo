...
public class CalendarDB {
     ...
     * 日历数据起始年
     */
    public static final int YEAR_BASE = 1901;

    /*
     * 日历数据长度
     */
    public static final int YEAR_NUM = 150;

    private static final String[] HanziNumber = {"零", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十"};

    public static final String[] WeekDayName = {"星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"};

    /**
     * 英文月
     */
    public static final String[] EnglishMonth = {"Jan.", "Feb.", "Mar.", "Apr.", "May", "Jun.", "Jul.", "Aug.", "Sept.", "Oct.", "Nov.", "Dec."};

    /**
     * 英文天
     */
    public static final String[] EnglishDay = {"1st", "2nd", "3rd", "4th", "5th", "6th", "7th", "8th", "9th", "10th", "11th", "12th", "13th",
     ...
     * 农历月
     */
    public static final String[] HanziMonth = {"正月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "冬月", "腊月"};
    /*
     * 农历日
     */
    public static final String[] LunarDayDict = {"初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十", "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八",
            "十九", "二十", "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十",};

    private static final String[] AstroNameDict = {"白羊座", "金牛座", "双子座", "巨蟹座", "狮子座", "处女座", "天秤座", "天蝎座", "射手座", "摩羯座", "水瓶座", "双鱼座"};

    /*
     * 天干名称
     */
    private static final String[] KanDict = {"甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸"};

    /*
     * 地支名称
     */
    private static final String[] ChiDict = {"子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"};

    /*
     * 生肖
     */
    private static final String[] ShengXiaoDict = {"鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪"};

    /*
     * 数组中每一个元素存放1901~2050期间每一年的闰月月份，取值范围0~12（0表示该年没有闰月）
     */
    private static int[] LunarLeapMonthDict = {0x00, 0x00, 0x05, 0x00, 0x00, 0x04, 0x00, 0x00, 0x02, 0x00, // 1910
     ...
     */
    private static int[] MonthDayDict = {0x0752, 0x0EA5, 0x164A, 0x064B, 0x0A9B, 0x1556, 0x056A, 0x0B59, 0x1752, 0x0752, // 1910
     ...
     * 1月1日 到农历一月一日有多少天
     */
    private static int[] AccDaysDict = {49, 38, 28, 46, 34, 24, 43, 32, 21, 40, 29, 48, 36, 25, 44, 33, 22, 41, 31, 50, 38, 27, 46, 35, 23, 43, 32, 22, 40,
            29, 47, 36, 25, 44, 34, 23, 41, 30, 49, 38, 26, 45, 35, 24, 43, 32, 21, 40, 28, 47, 36, 26, 44, 33, 23, 42, 30, 48, 38, 27, 45, 35, 24, 43, 32, 20,
            39, 29, 47, 36, 26, 45, 33, 22, 41, 30, 48, 37, 27, 46, 35, 24, 43, 32, 50, 39, 28, 47, 36, 26, 45, 34, 22, 40, 30, 49, 37, 27, 46, 35, 23, 42, 31,
            21, 39, 28, 48, 37, 25, 44, 33, 22, 40, 30, 49, 38, 27, 46, 35, 24, 42, 31, 21, 40, 28, 47, 36, 25, 43, 33, 22, 41, 30, 49, 38, 27, 45, 34, 23, 42,
    ...
    private static String[] solarTermNameDict = {"小寒", "大寒", "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏", "小满", "芒种", "夏至", "小暑", "大暑", "立秋", "处暑", "白露", "秋分",
            "寒露", "霜降", "立冬", "小雪", "大雪", "冬至",};

    private static int[] solarTermDict = {0x66, 0x44, 0x66, 0x56, 0x67, 0x67, 0x88, 0x89, 0x89, 0x99, 0x88, 0x87, // 1901
     ...
     * 公历平年月份天数
     */
    private static int[] MonthDays = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 31};

    private static int YearDays[] = {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334};

    /*
     * 公历平年月份天数
     */
    public static int getSolarMonthDays(int month) {
        return MonthDays[month];
    }

    public static int getSolarMonthDaysLeap(int month) {
    ...
    public static int getSolarYearDays(int year, int month, int day) {
        int position = month - 1;
        if (position >= YearDays.length || position<0) {
            position = 0;
        }
        int days = YearDays[position] + day;
        if (month > 2 && isLeapYear(year)) {
            ++days;
        }
        return days;
     ...
     * 农历年的天干地支名称
     */
    public static String getKanChiName(int year) {
        // 1901年干支为37
        int model = ((year - YEAR_BASE) + 37) % 60;
        return KanDict[model % 10] + ChiDict[model % 12];
    }
    ...
    public static int getLunarLeapMonth(int lunarYear) {
        int position = lunarYear - YEAR_BASE;
        if (position >= LunarLeapMonthDict.length) {
            position = 0;
        }
        return LunarLeapMonthDict[position];
    }

    /*
     * 查询农历某年某月有多少天 这个地方是有问题的 月不是正常的月
     */
    public static int getFakeLunarMonthDays(int lunarYear, int lunarMonth) {
        int position = lunarYear - YEAR_BASE;
        if (position >= MonthDayDict.length) {
            return 30;
        }
        return (MonthDayDict[position] & (1 << (lunarMonth - 1))) != 0 ? 30 : 29;
    }

    /*
     * 1月1日 到农历一月一日有多少天
     */
    public static int getAccDays(int year) {
        int position = year - YEAR_BASE;
        if (position >= AccDaysDict.length) {
            year = LunarDate.today().getYear();
        }
        return AccDaysDict[position];
    }
    ...
    public static boolean isLeapYear(int solarYear) {
        return (solarYear % 400 == 0) || ((solarYear % 100 != 0) && (solarYear % 4 == 0));
    }
    ...
    public static String getShengXiao(int lunarYear) {
        return ShengXiaoDict[(lunarYear - 1900) % 12];
    }

    public static String getShengXiaoByIndex(int index) {
        if (index >= ShengXiaoDict.length) {
            index = 0;
        }
        return ShengXiaoDict[index];
    }

    public static int getShengXiaoIndex(int lunarYear) {
        return (lunarYear - 1900) % 12;
    }

    /*
     * 查询生肖（beginWithLiChun 是否按立春来算生肖)
     */
    public static String getShengXiao(final SolarDate birth, boolean beginWithLiChun) {
        if (birth == null || !birth.isValidDate()) {
            return "";
        }
        if (!beginWithLiChun) {
            return getShengXiao(birth.toLunarDate().getYear());
        }

        int year = birth.getYear();
        // 得到此年立春这一天是2月几日
        int dayOfLiChun = getLichun(year);
        SolarDate dateOfLiChun = new SolarDate(year, 2, dayOfLiChun);
    ...
    // 得到次年立春是2月的几号
    public static int getLichun(int year) {
        int position = (year - YEAR_BASE) * 12 + 1;
        if (position >= solarTermDict.length) {
            position = 0;
        }
        return solarTermDict[position] >> 4;
    }

    /*
     * 获取星座名称
     */
    public static String getAstroName(int index) {
        if (index < 0 || index >= AstroNameDict.length) {
            return "未知";
        }
        return AstroNameDict[index];
    }

    /*{ "白羊座", "金牛座", "双子座", "巨蟹座", "狮子座", "处女座", "天秤座", "天蝎座", "射手座", "摩羯座", "水瓶座", "双鱼座" };
     * 获取星座名称
     */
    public static String getAstroName(int month, int day) {
        return getAstroName(getAstro(month, day));
    }
    ...
    public static int getAstro(int month, int day) {
        ...
        int[] beginDays = {21, 20, 21, 21, 22, 22, 23, 24, 24, 24, 23, 22};
    ...
    public static String formatLunarDate(int year, int month, int day) {
        ...
        sb.append(HanziNumber[year / 1000]);
        sb.append(HanziNumber[(year / 100) % 10]);
        sb.append(HanziNumber[(year / 10) % 10]);
        sb.append(HanziNumber[year % 10]);
        sb.append("年");
        sb.append(getLunarMonthName(month));
        sb.append(LunarDayDict[day - 1]);
        return sb.toString();
    }

    public static String formatLunarDate(int month, int day) {
        if (month < -12 || month > 12 || day > 30 || day < 1) {
            return "火星的生日";
        }
        StringBuilder sb = new StringBuilder();
        sb.append(getLunarMonthName(month));
    ...
    public static String getLunarMonthName(int month) {
        ...
        if (month < 0) {
            return "闰" + HanziMonth[Math.abs(month) - 1];
        } else {
            return HanziMonth[Math.abs(month) - 1];
        }
    }
    ...
    public static String getEnglishMonthName(int month) {
        ...
        return EnglishMonth[month - 1];
    }

    public static String getEnglishDayName(int day) {
        if (day < 0 || day > 31) {
            return "未知";
        }
        return EnglishDay[day - 1];
    }

    /*
     * 根据年月日返回是否是节气，如果不是节气日返回-1 否则返回节气的index
     */
    public static int GetTermBySolarDate(int year, int month, int day) {

        int data = solarTermDict[(year - YEAR_BASE) * 12 + month - 1];
        if (day < 15) {
    ...
    public static String GetTermBySolarDateName(int year, int month, int day) {
        return GetTermName(GetTermBySolarDate(year, month, day));
    }
    ...
    public static String GetTermName(int index) {
        if (index >= solarTermNameDict.length) {
            index = 0;
        }
        return (index > 23 || index < 0) ? "" : solarTermNameDict[index];
    }

    // 本月第一个节气的日子
    public static int getPreJieQi(int year, int month) {
        int position = (year - YEAR_BASE) * 12 + month - 1;
        if (position >= solarTermDict.length) {
            position = 0;
        }
        return solarTermDict[position] >> 4;
    ...
    // 本月第二个节气的日子
    public static int getAfterJieQi(int year, int month) {
        int position = (year - YEAR_BASE) * 12 + month - 1;
        if (position >= solarTermDict.length) {
            position = 0;
        }
        int pre = solarTermDict[position];
        int tem = 0x000f;
        return (pre & tem) + 15;
